...
architecture comportamento of decoderInstru is
  ...
  Hab_RD <= '1' when (opcode = LDA or opcode = SOMA or opcode = SUB) else '0';
  habFlag <= '1' when (opcode = CEQ) else '0';
  Op <= "01" when (opcode = SOMA) else "00" when (opcode = SUB) else "10" when (opcode = LDA or opcode = LDI) else "00";
